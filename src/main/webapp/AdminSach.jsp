...
<body>
    <div class="container-fluid position-relative d-flex p-0">
        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-secondary navbar-dark">
                ...
                <div class="d-flex align-items-center ms-4 mb-4">
                    ...
                    <div class="ms-3">
                        <h6 class="mb-0"><%=user.getHoten() %></h6>
                        <span>Admin</span>
                    </div>
                </div>
               		<%} %>
                <div class="navbar-nav w-100">
                    ...
                    <a href="./book" class="nav-item nav-link"><i class="fa fa-th me-2"></i>Sản phẩm</a>
                    <a href="./category" class="nav-item nav-link"><i class="fa fa-table me-2"></i>Danh mục</a>                    
                    <div class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown"><i class="far fa-file-alt me-2"></i>Trang khác</a>
                        <div class="dropdown-menu bg-transparent border-0">
                            <a href="./adminRegister" class="dropdown-item">Đăng ký</a>
                            <a href="./adminLogin" class="dropdown-item">Đăng nhập</a>                           
                        </div>
                    </div>
                </div>
            </nav>
        ...
        <div class="content">
                ...
                <div class="navbar-nav align-items-center ms-auto">
                    ...
                    <div class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown">
                            ...
                            <span class="d-none d-lg-inline-flex"><%=user.getHoten() %></span>
                        </a>
                        <div class="dropdown-menu dropdown-menu-end bg-secondary border-0 rounded-0 rounded-bottom m-0">
			...
			<%
				BookModel getBookById = (BookModel) request.getAttribute("getBookById");
			%>
              <!-- Form Start -->
                            ...
                            <div>
                                <div class="mb-3">
                                    ...
                                    <input type="text" name='txtms' class="form-control" 
                                    	value='<%=getBookById != null ? getBookById.getMasach() : ""%>' required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label" style="color: #fff;">
                                        Tên Sách
                                    ...
                                    <input type="text" name='txtts' class="form-control"
                                    	value='<%=getBookById != null ? getBookById.getTensach() : ""%>' required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label" style="color: #fff;">
                                        Tác Giả
                                    ...
                                    <input type="text" name='txttg' class="form-control"
                                    	value='<%=getBookById != null ? getBookById.getTacgia() : ""%>' required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label" style="color: #fff;">
                                        Số Lượng
                                    ...
                                    <input type="text" name='txtsl' class="form-control"
                                    	value='<%=getBookById != null ? getBookById.getSoluong() : ""%>' required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label" style="color: #fff;">
                                        Giá
                                    ...
                                    <input type="text" name='txtg' class="form-control"
                                    	value='<%=getBookById != null ? getBookById.getGia() : ""%>' required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label" style="color: #fff;">
                                        Mã Danh Mục
                                    ...
                                    <%
		                            	List<BookCategoryModel> listBookCategory = (List<BookCategoryModel>) request.getAttribute("listBookCategory");
		                            	for(BookCategoryModel bookCategory : listBookCategory) {		                   
											if (listBookCategory != null && listBookCategory.get(0).getMaloai().equals(bookCategory.getMaloai())) {
									%>
                                        <option value='<%=bookCategory.getMaloai()%>' selected><%=bookCategory.getMaloai()%></option>
                                        <%
										} else {
										%>
                                        <option value='<%=bookCategory.getMaloai()%>'><%=bookCategory.getTenloai()%></option>  
                                        <%}}%>                                    
                                    </select>
                                </div> 
                                <div class="mb-3">
                                    ...
                                    <form method="get" action="./upload" enctype="multipart/form-data">
                                    	<input class="form-control bg-dark" type="file" name='txta'
                                    		value='<%=getBookById != null ? getBookById.getAnh() : ""%>' required>
                                    </form>
                                </div>
                                <div style="display: flex; justify-content: flex-end;">
                                    <button type="submit" class="btn btn-primary"
                                        style="margin-right: 16px; width: 20%;" name='btn-update' value='sua'>
                                        Sửa
                                    </button>
                                    <button type="submit" class="btn btn-success" style=" width: 20%;" name='btn-add' value='them'>
                                        Thêm
                                    </button>
                                </div>
                                </div>
                ...
                <div class="bg-secondary text-center rounded p-4">
                    ...
                    <div class="table-responsive">
                        <table class="table text-start align-middle table-bordered table-hover mb-0">
                            ...
                            <%                          
                        		List<BookModel> listBook = (List<BookModel>) request.getAttribute("listBook");
                            ...
                            <tbody>
                                <tr>                                    
                                    ...
                                    <td><%=book.getGia() %></td>
                                    <td><%=book.getMaloai() %></td>
                                    <td><%=book.getAnh() %></td>
                                    <td style="text-align: center;">
                                    	<a class="btn btn-sm btn-primary" href="./book?id=<%=book.getMasach()%>&action=delete">
                                    		Xóa
                                    	</a>
                                    </td>
                                    <td style="text-align: center;">
                                    	<a class="btn btn-sm btn-success" href="./book?id=<%=book.getMasach()%>&action=choice">
                                    		Chọn
                                    	</a>
                                    </td>
                                </tr>
