...
<body>
        ...
        <header class="header header-cart">
            ...
            <div class="grid wide"> <!-- kích thước width = 1200px nên nó tự động căn = kích thước 1200px  -->
                ...
                <div class="header-with-search header-with-search-cart">
                    ...
                    <form action="./cart" method="get" class="header__search-cart">
                        <div class="header__search-input-wrap">
                            <input name="txtkey" type="text" class="header__search-input"
            ...
            <div class="grid wide">
                <div class="wrapper">
					...
					<%  
					    ...
					    if (cart != null) {                                
					        for (CartModel g : cart.listCart) {
					            double gia = g.getGia();
					            String formattedAmount = FormatCurrencyVND.formatCurrencyVND(gia);
					%>
					        <form action="./cart" method="get" class="cart-container" id="cart-form_<%=g.getMasach() %>">
					            ...
					            <div class="container-product-cart">
					                ...
					                <div class="product-cart">
					                    <img src="<%=g.getAnh() %>" alt="">
					                    <span><%=g.getTensach() %></span>
					                </div>
					                <div class="product-price">
					                    <span class="price-old">₫499.000</span>
					                    <span class="price-cur"><%= formattedAmount %></span>
					                </div>
					               <div class="product-sum">
					                    <button type="button" class="btn_cart btn_cart-sub" onclick="updateQuantity('<%=g.getMasach() %>', -1)">
					                        <i class="fa-solid fa-minus"></i>
					                    </button>
					                    <input id="quantity_<%=g.getMasach() %>" type="text" class="btn_cart btn_cart-quantity" name="quantity" value="<%=g.getSoluongmua()%>" readonly>
					                    <button type="button" class="btn_cart btn_cart-plus" onclick="updateQuantity('<%=g.getMasach() %>', 1)">
					                        <i class="fa-solid fa-plus"></i>
					                    </button>
					                    <input type="hidden" name="id" value="<%=g.getMasach() %>">
					                    <input type="hidden" name="action" value="update">
					                    <input type="hidden" name="quantity" id="hidden_quantity_<%=g.getMasach() %>" value="<%=g.getSoluongmua()%>">
					                </div>
					
					                <div class="product-money">
					                    <span><%= FormatCurrencyVND.formatCurrencyVND(g.getThanhTien()) %></span>
					                </div>
					                <div class="product-action">
					                    <a href="./cart?&id=<%=g.getMasach()%>">
					                        <span>Xóa</span>
					                    </a>
					                </div>
					            </div>
                        ...
                        <div class="container-buy-cart">
                            ...
                            <div class="checkout-buy-cart">
                                <div class="checkout-left-cart">
                                    ...
                                    <a href="./removeallcart">
                                    	<span class="remove-cart">Xóa Giỏ Hàng</span>
                                    </a>
                                </div>
                                <div class="checkout-right-cart">                             
                                    <span class="price-text">Tổng thanh toán:</span>
                                    <span class="price-checkout"><%=FormatCurrencyVND.formatCurrencyVND(cart.totalPrice()) %></span>
                                    <!-- <i class="fa-solid fa-angle-down"></i> -->
                                    <i class="fa-solid fa-chevron-up"></i> 
                                    <!-- Nút Mua Ngay -->
									<form action="./checkout" method="post" class="buy-now-form">	                                    
									    <button type="submit" class="btn btn--primary btn-checkout">Mua Ngay</button>
									</form>
                                    ...
                                    <div class="detail-checkout-container">
                                        ...
                                        <div class="detail-sum-product">
                                            ...
                                            <span><%=FormatCurrencyVND.formatCurrencyVND(cart.totalPrice()) %></span>
                                        </div>
    ...
    <script>
	    function updateQuantity(masach, change) {
	        // Lấy input số lượng dựa trên mã sách (masach)
	        const quantityInput = document.getElementById('quantity_' + masach);
	        let currentQuantity = parseInt(quantityInput.value, 10); // Lấy giá trị hiện tại của số lượng
	        currentQuantity += change; // Cộng thêm hoặc trừ đi theo giá trị 'change'
	
	        if (currentQuantity < 1) {
	            currentQuantity = 1; // Giới hạn số lượng tối thiểu là 1
	        }
	
	        // Cập nhật lại giá trị số lượng mới
	        quantityInput.value = currentQuantity;
	
	        // Cập nhật giá trị số lượng trong trường ẩn
	        document.getElementById('hidden_quantity_' + masach).value = currentQuantity;
	
	        // Gửi form để cập nhật giỏ hàng
	        document.getElementById('cart-form_' + masach).submit();
	    }
